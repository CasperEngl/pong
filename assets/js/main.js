...
let canvas = document.getElementById("canvas"),
	...
	paddleOne, paddleTwo, // Define paddles
...
class GameMessage {
	// Method for telling there's not enough players
	static GameFull() {
		alert('No more players can join');
	}

	// Method for telling there's not enough players
	static NeedMorePlayers() {
...
function joinRoom() {
	if (players.length === 0) {
		...
		paddleOne = new Paddle('bottom');
		// Spawn player one paddle
		paddles.push(paddleOne);
		...
		paddleTwo = new Paddle('top');
		// Spawn player two paddle
		paddles.push(paddleTwo);
	} else {
		return GameMessage.GameFull();
	}
...
socket.on('playerJoined', function (data) {
	...
	let position = ((data.id === 0) ? 'bottom' : 'top');
	paddles.push(new Paddle(data.id, position));
...
function update() {
	...
	if (mouse.x && mouse.y) {
		...
		*/
		...
		// console.log('Socket Client ' + JSON.stringify(playerMovement));

		// Emit current player movement
...
function collideAction(ball, p) {
	...
	if (paddles[playerID] && paddleHit === paddles[playerID].id) {
...
function updateScore() {
	if (players[playerID]) {
		...
		ctx.fillText("Score: " + players[playerID].points, 20, 20);
	}
}
...
function gameOver() {
	...
	let plural = ((players[playerID].points > 1 || players[playerID].points === 0) ? "points" : "point");
	ctx.fillText(`Game Over - You scored ${players[playerID].points} ${plural}!`, W / 2, H / 2 + 25);
	...
	*/
	...
	if (my >= startBtn.y && my <= startBtn.y + startBtn.h) {
		if (players.length > 0) {
			...
			if (players.length === 1) {
				paddles.push(new Paddle(playerID + 1, 'top', W * 5));
			}
